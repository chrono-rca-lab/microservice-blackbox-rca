...
def test_step_up():
    ...
    res = detect_change_points_bootstrap(series, baseline, k=0.5, rng_seed=42)
...
def test_step_down():
    ...
    res = detect_change_points_bootstrap(series, baseline, rng_seed=42)
    assert len(res.change_points) >= 1
...
def test_no_change():
    ...
    res = detect_change_points_bootstrap(series, baseline, confidence_level=0.999, rng_seed=42)
...
def test_empty():
    res = detect_change_points_bootstrap(np.array([]), np.zeros(50), rng_seed=42)
...
def test_single_sample():
    ...
    baseline = np.zeros(50)
    res = detect_change_points_bootstrap(np.array([5.0]), baseline, rng_seed=42)
    assert isinstance(res.change_points, list)
